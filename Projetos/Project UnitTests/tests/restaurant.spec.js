...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(createMenu({teste1: 1, teste2: 2}).fetchMenu()).toMatchObject({
      teste1: 1,
      teste2: 2
    });
    ...
    // ```
    const objetoRetornado2 = createMenu({ food: {'aaa': 'bbb'}, drink: {'ccc': 'ddd'} });
    expect(objetoRetornado2.consumption).toStrictEqual([])
    ...
    // ```
    const objetoRetornado3 = createMenu({ food: {'coxinha': 3}, drink: {'cerveja': 8} });
    expect(objetoRetornado3.order('coxinha')).toStrictEqual(['coxinha']);
